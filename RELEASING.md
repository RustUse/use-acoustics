...
1. Create or reuse a crates.io API token with publish access for `use-acoustics`.
...
- `crate = all` or `crate = use-acoustics`
- `dry_run = true`
...
After the first manual crates.io release for `use-acoustics` exists, the repository can use the
...
- Purpose: confirms `use-acoustics` already exists on crates.io, then runs `release-plz release`

Real release-plz publishes still require `CARGO_REGISTRY_TOKEN` unless the repository later moves
to trusted publishing.

## Local dry-run example

```sh
cargo publish -p use-acoustics --dry-run
```
